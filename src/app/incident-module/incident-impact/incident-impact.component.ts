...
@Component({
  ...
  styleUrls: ['./incident-impact.component.css']
})
export class IncidentImpactComponent implements OnInit {
  ...
  getIncident() {
    ...
    if(this.info.test !== undefined && this.info.test !== null){
      ...
      this.businessDTOs = this.info.test.securityDTOs;
  ...
  saveBusinessRisk() {
    this.loading = true;
    let url = APP_CONFIG.saveIMSecurity;
    this.httpClient.post(url, this.imBusinessRiskDTO)
      .subscribe((data: any) => {
        this.loading = false;
        this.dialogService.open("Info", "Security Risk has been created.", false, "OK", "OK")
          .then((result: any) => {
